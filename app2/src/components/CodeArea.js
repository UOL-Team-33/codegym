...
class CodeArea extends React.Component {
	...
	parse_string(content, chars_list) {
		let content_copy = [];
	...
	recursive_parse(el, chars) {
		if (el.nodeName !== "#text") {

			if (el.innerHTML === el.innerText) {
				el.innerHTML = this.parse_string(el.innerHTML, chars)
			} else {
				for (let i = 0; i < el.childNodes.length; i++) {
					this.recursive_parse(el.childNodes[i], chars)
				}
			}
		} else {
			let span = document.createElement("span");
			let newHTML = this.parse_string(el.textContent, chars)
			span.innerHTML = newHTML
			el.parentNode.replaceChild(span, el)
		}
	}

	augment_code(html_code) {
		let div = document.createElement("div");
		div.innerHTML = html_code
		let chars_to_type = []

		this.recursive_parse(div, chars_to_type)
		this.setState({
			chars_to_type
		})
	...
	toggle_char_class = (key_index, clasname) => {
		const key = this.containerRef.current.querySelector('#char-' + key_index);
		console.log(key)
		key.classList.toggle(clasname)
	}
	...
	componentDidMount() {
		...
		let final_code = this.augment_code(Prism.highlight(this.state.raw_code, Prism.languages.javascript, 'javascript'))
