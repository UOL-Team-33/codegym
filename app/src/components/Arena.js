...
const Arena = ({
  ...
  const handleKeyDown = event => {
    ...
    if (ready === GameState.Started) {
      ...
      if (currentKey === 'Backspace') {
        ...
        // Save the typed characters to a variable as we can only read
        // the new characters on the next re-render, but we want to use them now
        // This is to fix -infinity and negative accuracy
        const newCharsTyped = [...charsTyped, currentKey];
        setCharsTyped(newCharsTyped);

        onStatsChange({
          correct: Math.min(stats.correct + char_success, charsToType.length),
          wrong: stats.wrong + !char_success,
          charsToTypeLength: newCharsTyped.length,
          charsTypedLength: newCharsTyped.length,
          maxIndex: maxIndex
        });
      }
    } else {
  ...
  useEffect(() => {
    let [final_code, chars_to_type] = augment_code(
      Prism.highlight(code, Prism.languages[language], language_map[language])
    );
